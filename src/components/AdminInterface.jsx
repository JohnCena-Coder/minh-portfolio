      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-end mb-10 border-b border-gray-200 pb-4">
            <div>
                ...
                <p className="text-gray-400 text-sm mt-1">Hệ thống quản lý nội dung</p>
            </div>
        ...
        {activeTab === 'info' && (
            <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 animate-in fade-in slide-in-from-bottom-2">
                <h2 className="text-xl font-bold mb-6">Ảnh Giới Thiệu (Info)</h2>
                
                {/* Khu vực hiển thị ảnh hiện tại */}
                {infoImage ? (
                    <div className="mb-8">
                        <p className="text-sm text-gray-500 mb-2">Ảnh đang hiển thị:</p>
                        <div className="relative inline-block group">
                            ...
                            <button 
                                ...
                                className="absolute -top-3 -right-3 bg-red-500 text-white p-2 rounded-full shadow-md hover:bg-red-600 hover:scale-110 transition flex items-center justify-center"
                                title="Xóa ảnh này"
                            >
                                <Trash2 size={20} />
                            </button>
                    ...
                    <div className="p-8 mb-8 text-center border-2 border-dashed rounded-lg bg-gray-50 text-gray-400">
                        Hiện chưa có ảnh nào.
                    </div>
                )}

                {/* Form Upload */}
                <form onSubmit={handleSaveInfo} className="max-w-md">
                    <label className="block text-sm font-bold mb-2">Cập nhật ảnh mới</label>
                    <div className="flex gap-4 items-center">
                        ...
                        <button disabled={loading} className="px-6 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition font-medium whitespace-nowrap">
                            {loading ? <Loader2 className="animate-spin"/> : 'Lưu Ảnh'}
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div>
                {!isEditing && (
                     <button onClick={() => setIsEditing(true)} className="flex items-center px-5 py-3 mb-8 text-white bg-black rounded-lg shadow-lg hover:bg-gray-800 hover:translate-y-[-2px] transition-all">
                        <Plus className="w-5 h-5 mr-2" /> Thêm Dự Án Mới
                    </button>
                )}
                ...
                {isEditing && (
                    <div className="p-6 mb-8 bg-white rounded-xl shadow-lg border border-gray-100 animate-in fade-in slide-in-from-bottom-4">
                        <div className="flex justify-between items-center mb-6">
                             <h2 className="text-xl font-bold">{editId ? 'Chỉnh Sửa Dự Án' : 'Thêm Dự Án Mới'}</h2>
                             <button onClick={() => {setIsEditing(false); resetForm();}} className="text-gray-400 hover:text-black">Đóng</button>
                        </div>
                        ...
                        <form onSubmit={handleSaveProject} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-sm font-bold mb-2">Phân loại</label>
                            ...
                            <div className="p-6 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                                <div className="mb-6">
                                    <label className="block text-sm font-bold mb-2">1. Ảnh Bìa (Thumbnail)</label>
                                ...
                                <div>
                                    <label className="block text-sm font-bold mb-2">2. Gallery (Nhiều ảnh)</label>
                                    ...
                                    <p className="text-xs text-gray-400 mt-2">Giữ Ctrl để chọn nhiều ảnh cùng lúc.</p>
                                </div>
                            </div>

                            <div className="flex justify-end gap-3 pt-4 border-t">
                                <button type="button" onClick={() => {setIsEditing(false); resetForm();}} className="px-6 py-2 text-gray-600 hover:bg-gray-100 rounded-lg transition font-medium">Hủy</button>
                                <button disabled={loading} type="submit" className="px-8 py-2 text-white bg-black rounded-lg hover:bg-gray-800 transition font-bold shadow-lg">
                                    {loading ? <Loader2 className="animate-spin" /> : 'LƯU DỰ ÁN'}
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div className="space-y-4">
                    {projects.map(p => (
                        <div key={p.id} className="group flex items-center justify-between p-4 bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition duration-300">
                            ...
                            <div className="flex items-center gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => { setEditId(p.id); setFormData(p); setIsEditing(true); }} className="p-2 text-gray-500 hover:text-black hover:bg-gray-100 rounded-full transition" title="Sửa">
                                    <Edit size={18} />
                                </button>
                                <button onClick={() => handleDeleteProject(p.id)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-full transition" title="Xóa">
                                    <Trash2 size={18} />
                                </button>
                            </div>
                        </div>
                    ...
                    {projects.length === 0 && (
                        <div className="text-center py-16 bg-white rounded-xl border border-dashed">
                            <p className="text-gray-400 mb-4">Chưa có dự án nào.</p>
                            <button onClick={() => setIsEditing(true)} className="text-black font-bold underline">Thêm dự án đầu tiên</button>
                        </div>
                    )}
                </div>
            </div>
